...
public interface CourserRepository extends JpaRepository<CourseModel, UUID>, JpaSpecificationExecutor<CourseModel> {
    @Query(value = "select case when count(tcu) > 0 THEN true ELSE false END FROM tb_courses_users tcu WHERE tcu.course_id = :couseId and tcu.user_id= :userId", nativeQuery = true)
    ...
    void saveCourseUser(@Param("courseId") UUID couseId, @Param("userId") UUID userId);
